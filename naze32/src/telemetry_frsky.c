...
static void sendAccel(void)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        serialize16(((float)accSmooth[i] / sensor_set.acc.acc_1G) * 1000);
    }
}
...
{
    uint32_t seconds = pif_cumulative_timer1ms / 1000;
...
{
    if (mcfg.telemetry_port == TELEMETRY_PORT_UART) {
        serialInit(UART_PORT_1, 9600, (hw_revision == NAZE32_SP && !mcfg.spektrum_sat_on_flexport) ? UART_PORT_3 : UART_PORT_NONE);
    }
}
...
void handleFrSkyTelemetry(void)
{
    ...
    uint32_t now = pif_cumulative_timer1ms;
