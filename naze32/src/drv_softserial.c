...
static void softSerialGPIOConfig(GPIO_TypeDef *gpio, uint16_t pin, GPIO_Mode mode)
{
    ...
    gpio_cfg.pin = pin;
    gpio_cfg.mode = mode;
    gpio_cfg.speed = Speed_2MHz;
    gpioInit(gpio, &gpio_cfg);
}
...
{
    softSerial->rxBufferSize = SOFT_SERIAL_BUFFER_SIZE;
    softSerial->rxBufferTail = 0;
    softSerial->rxBufferHead = 0;

    softSerial->txBufferSize = SOFT_SERIAL_BUFFER_SIZE;
    softSerial->txBufferTail = 0;
    softSerial->txBufferHead = 0;
}

void initialiseSoftSerial(softSerial_t *softSerial, uint8_t portIndex, uint32_t baud, uint8_t inverted)
{
    softSerial->port.p_param = (void*)softSerialVTable;
...
{
    if (softSerial->rxBufferTail >= softSerial->rxBufferSize - 1) {
        softSerial->rxBufferTail = 0; //cycling the buffer
    } else {
        softSerial->rxBufferTail++;
    }
}
...
void processTxState(softSerial_t *softSerial)
{
    ...
    if (!softSerial->isTransmittingData) {
        ...
        // data to send
        byteToSend = softSerial->txBuffer[softSerial->txBufferTail++];
        if (softSerial->txBufferTail >= softSerial->txBufferSize) {
            softSerial->txBufferTail = 0;
        }
...
void extractAndStoreRxByte(softSerial_t *softSerial)
{
    ...
    softSerial->rxBuffer[softSerial->rxBufferTail] = rxByte;
...
uint8_t softSerialTotalBytesWaiting(serialPort_t *instance)
{
    ...
    if (softSerial->rxBufferTail == softSerial->rxBufferHead) {
        return 0;
    }

    if (softSerial->rxBufferTail > softSerial->rxBufferHead) {
        availableBytes = softSerial->rxBufferTail - softSerial->rxBufferHead;
    } else {
        availableBytes = softSerial->rxBufferTail + softSerial->rxBufferSize - softSerial->rxBufferHead;
    }
...
{
    if (softSerial->rxBufferHead < softSerial->rxBufferSize - 1) {
        softSerial->rxBufferHead++;
    } else {
        softSerial->rxBufferHead = 0;
    }
}
...
uint8_t softSerialReadByte(serialPort_t *instance)
{
    ...
    b = softSerial->rxBuffer[softSerial->rxBufferHead];
...
void softSerialWriteByte(serialPort_t *s, uint8_t ch)
{
    ...
    softSerial->txBuffer[softSerial->txBufferHead] = ch;
...
bool isSoftSerialTransmitBufferEmpty(serialPort_t *instance)
{
    ...
    return softSerial->txBufferHead == softSerial->txBufferTail;
}
